...
import { StyledRoutinesEdit } from './style';

export default function RoutinesEdit() {
    /** Url Params */
    const { id } = useParams();
    /** Navigate effect */
    ...
    /** Routines Context */
    const { routine, fetchRoutine, updateRoutine } = useContext(RoutinesContext);
    ...
    // componentDidUpdate: routine, categories, exercises. Actualizamos las rutinas del formulario
    useEffect(() => {
        if(routine) {
        ...
        if(routine && categories && exercises) {
            const exs = routine.exercises.map(ex => ({
                category_id: ex.category_id,  // Id de la categoría
                exercise_id: ex.exercise_id,                    // Id del ejercicio
                categories: categories,          // Opciones de categorías disponibles según el filtro
                exercises: exercises.filter(e => e.category_id === ex.category_id),            // Opciones de ejercicios disponibles según la categoría elegida y el filtro
                // order: routineExercises.length,      // Orden del ejercicio (se seteará en el submit)
                series: ex.series,                              // Número de series
                repetitions: ex.repetitions,                         // Repeticiones por serie
                contraction: ex.contraction,                         // Tiempo contrayendo (0 para que no haya timer)
                rest: ex.rest,                                // Tiempo descansando (0 para que no haya timer)
                weight: ex.weight,                              // Peso (en Kg)
            }));

            setRoutineExercises(exs);
        }
    }, [routine, categories, exercises]);
    ...
    const onSubmit = async event => {
        ...
        const ok = await updateRoutine({ id: routine.id, name, description, exercises: exs });
        if(ok) {
            navigate("/routines");
        }
    }
    ...
    return (
        <StyledRoutinesEdit>
            ...
            <form onSubmit={onSubmit}>
                ...
                <PrimaryButton>Editar rutina</PrimaryButton>
            </form>
        </StyledRoutinesEdit>
    );
}
